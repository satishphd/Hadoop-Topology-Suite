...
public void spatialjoin(Node tempNode, RTree tree1,TIntProcedure v) 
{
	int entrycount = tempNode.entryCount;
	for(int i = 0; i< entrycount; i++)
	{
			tempNode = getNode(tempNode.ids[i]);	
			Rectangle rect = new Rectangle(tempNode.mbrMinX,tempNode.mbrMinY,tempNode.mbrMaxX,tempNode.mbrMaxY);
			tree1.intersects(rect, v);
			spatialjoin(tempNode,tree1,v);
	}
}

public void joinRtree(Node aNode, Node bNode,TIntProcedure v)
{
	for(int i = 0; i< aNode.entryCount; i++)
	{
		for(int j = 0; j< bNode.entryCount; j++)
		{
			if(Rectangle.intersects(aNode.mbrMinX, aNode.mbrMinY, aNode.mbrMaxX, aNode.mbrMaxY,
			bNode.mbrMinX, bNode.mbrMinY, bNode.mbrMaxX, bNode.mbrMaxY))
			{
			  ...
			  {
				  joinRtree(aNode,getNode(bNode.ids[j]),v);
			  }
			  else if(bNode.isLeaf())
			  {
				  joinRtree(getNode(aNode.ids[i]),bNode,v);
			  }
			  else
			  {
				  joinRtree(getNode(aNode.ids[i]), getNode(bNode.ids[j]), v);
			  }
			}
		}
	}
